...
function crear(titulo = "", descripcion = "", estado = "pendiente") {
    ...
    if (titulo.length > 5) {
        ...
        console.log(chalk.inverse.green("Tarea creada con éxito!"))

    } else {
        console.log(chalk.inverse.redBright("Tenes que ingresar un título y debe tener minimo 5 caracteres"));
    }
    ...
    function borrar(titulo) {
        ...
        let tareasActualizadas = tareas.filter (
         ...
         if (tareas.length !== tareasActualizadas.length) {
                ...
                console.log(chalk.inverse.greenBright("Tarea Borrada Viejita!!"));
         } else {
                console.log(chalk.inverse.red("No esta la tarea, pero si el potrillo!"));
            }
    }
